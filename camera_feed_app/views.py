...
class CameraSwitchTimeViewSet(viewsets.ViewSet):
    """
    A ViewSet for listing, creating, and updating CameraSwitchTime entries.
    """
    
    @swagger_auto_schema(
        operation_summary="List all CameraSwitchTime entries",
        responses={200: CameraSwitchTimeSerializer(many=True)}
    )
    def list(self, request):
        """
        List all CameraSwitchTime entries.
        """
        queryset = CameraSwitchTime.objects.all()
        serializer = CameraSwitchTimeSerializer(queryset, many=True)
        return Response({"results": serializer.data})
    
    @swagger_auto_schema(
        operation_summary="Create a new CameraSwitchTime entry",
        request_body=CameraSwitchTimeSerializer,
        responses={201: "Time saved for the camera switching", 400: "Bad request"}
    )
    def create(self, request):
        """
        Create a new CameraSwitchTime entry.
        """
        serializer = CameraSwitchTimeSerializer(data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response({"message": "Time saved for the camera switching"}, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    
    @swagger_auto_schema(
        operation_summary="Update an existing CameraSwitchTime entry",
        request_body=CameraSwitchTimeSerializer,
        responses={200: "Time updated successfully", 400: "Bad request", 404: "Not found"}
    )
    def update(self, request, pk=None):
        """
        ...
        """
        try:
            # Retrieve the CameraSwitchTime instance by id (pk)
            camera_switch_time = CameraSwitchTime.objects.get(pk=pk)
        except CameraSwitchTime.DoesNotExist:
            return Response({"detail": "Not found."}, status=status.HTTP_404_NOT_FOUND)
        
        # Serialize the instance with the new data
        serializer = CameraSwitchTimeSerializer(camera_switch_time, data=request.data, partial=False)
        if serializer.is_valid():
            serializer.save()  # Save the updated instance
            return Response({"message": "Time updated successfully", "data": serializer.data}, status=status.HTTP_200_OK)
